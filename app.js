...
function calculateSleepMetrics({ date, bedtime, wakeTime, fallAsleepMin }) {
...
async function submitSleepEntry(event) {
  ...
  if (metrics.totalSleepMin < 60 || metrics.totalSleepMin > 900) {
    alert('Thời lượng ngủ cần nằm trong khoảng 1 đến 15 tiếng.');
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.message || 'Không thể lưu dữ liệu giấc ngủ');
    }

    closeSleepEntryModal();
    await loadDashboardData();
    await loadSleepData('week');
    alert('Đã lưu dữ liệu giấc ngủ và cập nhật dashboard.');
  } catch (err) {
    alert(err.message || 'Không thể lưu dữ liệu giấc ngủ.');
  } finally {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (sleepEntryModal) {
    sleepEntryModal.addEventListener('click', (event) => {
      if (event.target === sleepEntryModal) closeSleepEntryModal();
    });
  }
...
async function loadDashboardData() {
  ...
  try {
    ...
    if (!lastRecord) {
      ...
      if (entryTitle) entryTitle.textContent = 'Ghi nhận giấc ngủ đầu tiên';
      if (entryCopy) entryCopy.textContent = 'Nhập giờ ngủ tối qua để AuraSleep bắt đầu phân tích bằng dữ liệu thật của bạn.';
      return;
    }
    ...
    if (entryTitle) entryTitle.textContent = 'Cập nhật giấc ngủ mới';
    if (entryCopy) {
      const recordDate = formatDateOnly(lastRecord.date);
      entryCopy.textContent = `Bản ghi gần nhất: ${recordDate}, ${formatSleepDuration(lastRecord.totalSleepMin)}, điểm ${lastRecord.sleepScore || '--'}/100.`;
    }
  } catch (err) {
